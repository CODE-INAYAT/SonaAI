  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (tab === "signup") {
        ...
        // If auto-logged in (e.g. email confirmation disabled), sign out to force manual login
        if (authData.session) {
        ...
        router.push(
          joinCode
            ? `/meeting?join=${encodeURIComponent(joinCode)}`
            : "/dashboard",
        );
      }
    } catch (error: any) {
    ...
    >
      ...
      >
        ...
        {!isResettingPassword && (
          <>
            {/* Social Logins */}
            {(features.auth.enableGoogleAuth || features.auth.enableGithubAuth) && (
              <div className="mb-6 flex gap-3">
                {features.auth.enableGoogleAuth && (
                  <button
                    type="button"
                ...
                {features.auth.enableGithubAuth && (
                  <button
                    type="button"
            ...
            {(features.auth.enableGoogleAuth || features.auth.enableGithubAuth) && (
              <div className="relative mb-6">
                ...
                <div className="relative flex justify-center text-xs">
                  <span className="bg-white px-2" style={{ color: "#707070" }}>
        ...
        {isResettingPassword ? (
          <form onSubmit={handleResetPassword}>
            ...
            <motion.button
              ...
              style={{
                backgroundColor: email && !isLoading ? "#031A10" : "#E5E7EB",
                color: email && !isLoading ? "#D0FFA2" : "#9CA3AF",
              }}
              whileHover={
                email && !isLoading
                  ? {
                        ...
                        <ul className="space-y-1.5 text-xs text-gray-600">
                          {[
                            ...
                            <li key={i} className="flex items-center gap-2">
                              ...
                              <span
                                className={
                                  req.met ? "text-green-700 font-medium" : ""
                                }
                              >
                                {req.text}
                              </span>
                    ...
                    >
                      {(["student", "teacher"] as UserRole[]).map((r) => (
                        <button
                          ...
                          style={{
                            background: role === r ? "#031A10" : "transparent",
                            color: role === r ? "#D0FFA2" : "#707070",
                            letterSpacing: "-0.32px",
                          }}
                          aria-pressed={role === r}
                        >
      ...
      {/* Attribution */}
      <p className="mt-6 text-xs" style={{ color: "#707070" }}>
        Powered by{" "}
        <a
