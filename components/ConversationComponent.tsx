...
export default function ConversationComponent({
  ...
  const handleMicToggle = useCallback(async () => {
    ...
    try {
      await localMicrophoneTrack.setMuted(isEnabled);
  ...
  const handleVideoToggle = useCallback(async () => {
    if (isScreenShareEnabled) {
      // Don't allow camera if screen share is active (single video track limit)
      return;
    }
    if (localCameraTrack) {
      try {
        await localCameraTrack.setMuted(isVideoEnabled);
      } catch (error) {
  ...
  const handleRaiseHandToggle = useCallback(() => {
    ...
    if (rtmClient && agoraData?.channel) {
      rtmClient.publish(agoraData.channel, JSON.stringify({ 
        type: "raise_hand", 
        uid: agoraData.uid, 
        raised: nextState 
      })).catch(() => {});
    }
  }, [isHandRaised, rtmClient, agoraData]);
